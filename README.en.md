...
Two different things live in this repository, so there are two sets of terms.

- **The code** — the application, the generation and validation scripts, the index builders, the tests and the configuration: [MIT](./LICENSE), © 2026 SUPWILSOFT LLC. Use it, fork it, change it.
- **The digests under `NEWS/`** — see [LICENSE-CONTENT](./LICENSE-CONTENT). These are machine-generated summaries of reporting published elsewhere, each linking to its sources. The underlying articles belong to their publishers rather than to this project, so an MIT grant over them would be granting rights this project does not hold. Read them, link to them, quote them with attribution. For bulk redistribution or use as a training set, open an issue and ask first.

The digests are generated by a model and validated mechanically, not by a human editor. They will contain errors and are not a substitute for reading the linked sources.
