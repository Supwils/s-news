#!/usr/bin/env bash
#
# 使用 Cursor CLI 执行供应链与制造新闻日报 command
# 将 .cursor/commands/supply-chain-news.md 作为 context，按其中流程生成日报并写入 NEWS/supply-chain/
#
...
COMMAND_FILE=".cursor/commands/supply-chain-news.md"
INSTRUCTION="今日日期必须使用：${TODAY}（${TODAY_CN}），文件名与标题中的日期一律为此日，不得使用 2025 年或其他年份。严格按照 @${COMMAND_FILE} 的完整流程执行：按优先级做多轮实时搜索（Web Search），筛选排序后按该文件中的 Markdown 模板整理，最后将中文版与英文版日报分别写入 NEWS/supply-chain/zh/ 和 NEWS/supply-chain/en/ 对应日期的 md 文件。不要省略任何步骤。只允许创建或覆盖本主题当天对应的中文和英文日报文件；不要修改其他日期、其他主题、脚本、配置或索引文件。若信息不足，宁可减少条目，也不要编造链接、日期或数字。"
...
bash "$SCRIPT_DIR/assert-digest.sh" "supply-chain" "供应链与制造新闻日报" "supply-chain-digest" "$TODAY"
