...
export function TodayPulse({ latestEntries, latestDate, className }: TodayPulseProps) {
  ...
  const topicLabel =
    visibleTopics.length > 0
      ? `${visibleTopics.map((topic) => topic.shortLabel).join("、")} ${copy.home.todayPulse.topicReadySuffix}`
      : copy.home.todayPulse.fallbackLead;

  return (
    <section
      className={[
        "overflow-hidden rounded-[28px] border border-[var(--color-border)] bg-[var(--color-surface)]/95 p-5 shadow-[var(--shadow-card)] sm:p-6",
        className ?? "",
      ].join(" ")}
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-sm uppercase tracking-[0.28em] text-[var(--color-text-muted)]">{copy.home.todayPulse.badge}</p>
        ...
        <div className="rounded-full border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-4 py-2 text-sm text-[var(--color-text-secondary)]">
          {latestDate ? formatDisplayDate(latestDate) : copy.home.todayPulse.latestLabel}
        </div>
      </div>

      <div className="mt-6 rounded-[22px] border border-[var(--color-border)] bg-[var(--color-surface-muted)]/80 px-4 py-5 sm:px-5">
        <div className="relative min-h-[68px] overflow-visible sm:min-h-[74px]">
          <div className="absolute inset-x-4 top-3 h-px bg-[var(--color-border-strong)]" />
            ...
            {visibleTopics.map((topic, index) => (
              <div key={topic.key} className="flex min-w-0 flex-1 items-center justify-center">
                <div className="flex flex-col items-center gap-2.5">
                  <span
                    className="pulse-node flex h-3 w-3 rounded-full border border-[var(--color-border)] bg-[var(--color-surface)]"
                    style={{
                  ...
                  />
                  <span
                    className="inline-flex items-center gap-2 rounded-full border border-[var(--color-border)] bg-[var(--color-surface)] px-2.5 py-1 text-xs text-[var(--color-text-secondary)]"
                  >
